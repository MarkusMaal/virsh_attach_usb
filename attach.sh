...
id=0
echo
echo -e "\e[34;1mQuick and dirty USB attachment tool\e[0m"
echo
for dev in "${names[@]}"; do
	idx="$id"
...
echo
echo -en "Please enter a \e[32mnumber\e[0m corresponding to the device you want to attach or type \e[32mq\e[0m to quit: "
read -p "" ans
([ "$ans" == "Q" ] || [ "$ans" == "q" ]) && exit
clear
echo
...
read -p "Attach device to (name of VM): " vm
([ "$vm" == "Q" ] || [ "$vm" == "q" ]) && exit
...
echo -e "\e[34;1mQuick and dirty USB attachment tool\e[0m"
echo
sudo virsh attach-device --domain $vm --file "$detach_fname" --current 2>/dev/null
